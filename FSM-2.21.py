...
class StateManager:
    def __init__(self):
        ...
        self.state_triggers = {
            ...
            "S6": {"point_name": "焊机电源", "condition": lambda d, t: d < t},
            "S7": {"point_name": "电压电流旋钮", "condition": lambda d, t: d < t},
            "S8": {
                "logic": "AND",  # 和关系
                "conditions": [
                    {"point_name": "焊把钳下端", "condition": lambda d, t: d < t},
            ...
            "S10": {"point_name": "焊机电源", "condition": lambda d, t: d < t},
            "S11": {"point_name": "墙壁电源", "condition": lambda d, t: d < t,"pre_condition": "S5"},
            "S12": {
                "logic": "AND",  # 和关系
                "conditions": [
                    {"point_name": "焊把钳下端", "condition": lambda d, t: d < t},
            ...
            "S13": {"point_name": "锤子", "condition": lambda d, t: True},  
            # "S14": {"auto_transition_to": "S14" },  #自动转移到 S14终止状态
        }
    
    ##########判断是否满足多点规则，超出时间限制判断为不满足
    def clear_expired_cache(self, current_time, timeout=15):
        """
    ...
    def check_and_trigger(self, row, current_state_name):
        ...
        # 使用文件中的时间戳
        current_time = row["contact_time"]  # 文件中的时间戳
        point_name = row["point_name"]  # 当前检测点名称
        self.clear_expired_cache(current_time, timeout=15)   #清除超时缓存
        for state_key, rule in self.state_triggers.items():
            logic = rule.get("logic", "AND")
            conditions = rule.get("conditions", [])
            pre_condition = rule.get("pre_condition")
            auto_transition_to = rule.get("auto_transition_to")
            # 单点规则处理
            if "point_name" in rule:
                ...
                if (
                    ...
                    self.completed_states.add(state_key)
                continue
            ...
            # 初始化缓存
                if state_key not in self.partial_conditions:
                    self.partial_conditions[state_key] = {
                        "points": set(),
                        "timestamp": current_time  # 记录缓存创建时间
                    }
                ...
                # 检查前置条件
                    if pre_condition and current_state_name != pre_condition:
                        # 前置条件未满足，触发替代状态
                        print(f"[WARNING] 前置条件 {pre_condition} 未满足，跳过状态 {state_key}")
                        continue  # 跳过触发
...
class FSM:
    ...
    def transition_to(self, next_state_key, trigger_data=None):
        if next_state_key in self.states:
            ...
            if (current_time - last_entry_time).total_seconds() < 1:
                return  # 不满足条件，跳过转移
            self.state_manager.completed_states.add(self.current_state.name)
            log_entry = {
    ...
    #         df["contact_time"] = pd.to_datetime(df["contact_time"], unit="s").dt.strftime("%Y-%m-%d %H:%M:%S.%f")
    #     # 保存到 CSV 文件
...
if __name__ == "__main__":
    fsm = FSM()
    input_file = r"D:\\QTproject\\hr_rtls_pc-master\\Logs\1.21交职UWB报警实验\\双手1，阈值0.5\\接触检测结果1.1.csv"
    output_file = r"D:\\QTproject\\hr_rtls_pc-master\\Logs\1.21交职UWB报警实验\\双手1，阈值0.5\\状态转移日志1.csv"
